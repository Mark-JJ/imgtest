...
//			Blob b = null;
	...
	private static final String UPDATA = "Update member42 set Password=?,Name=?,Address=?,Phone=?,Birthday=?,Weight=? where memberId=?";


	public MemberBean updataMember(MemberBean bean) throws SQLException {
		MemberBean result = null;
		try(
			Connection conn = ds.getConnection();
			PreparedStatement stmt = conn.prepareStatement(UPDATA);
		) {
			...
			stmt.setString(7, bean.getMemberId());
